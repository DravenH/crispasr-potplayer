...
"%CSC%" -nologo -optimize+ -out:"bin\whisper-faster.exe" "src\XxlShim.cs" || ( popd & exit /b 1 )
"%CSC%" -nologo -optimize+ -target:winexe -out:"bin\CrispASR-PotPlayer-Setup.exe" "-resource:bin\whisper-faster.exe,wf.exe" "-resource:config\shim.ini.example,ini.txt" -reference:System.Windows.Forms.dll -reference:System.Drawing.dll -reference:System.IO.Compression.dll -reference:System.IO.Compression.FileSystem.dll "src\Installer.cs" "src\Download.cs" || ( popd & exit /b 1 )
